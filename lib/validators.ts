...
//schema foe payment method
export const paymentMethodSchema = z
  .object({
    type: z.string().min(1, "Payment method is required"),
  })
  .refine((data) => PAYMENT_MENTHODS.includes(data.type), {
    path: ["type"],
    message: "Invalid payment method",
  });

//schema for insertng order
export const insertOrderSchema = z.object({
  ...
  taxPrice: currency,
  totalPrice: currency,
  paymentMethod: z.string().refine((data) => PAYMENT_MENTHODS.includes(data), {
    message: "Invalid payment method",
  }),
